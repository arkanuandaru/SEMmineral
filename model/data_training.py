...
#START ADDING DATA TO THE DATAFRAME
    ...
    df['file_name'] = image   #Capture image name as we read multiple images
...
#Gerate OTHER FEATURES and add them to the data frame
    ...
    # # extract xy coord
    yx_coords = np.column_stack(np.where(img >= 0))
    yx_df = pd.DataFrame(yx_coords, columns =["y", "x"])
    
    df2 = df.join(yx_df)

######################################                    
#Update dataframe for images to include details for each image in the loop
    image_dataset = image_dataset.append(df2)
...
mask_path = "labelled_data/"    
for mask in os.listdir(mask_path):  #iterate through each file to perform some action
    print(mask)
    
    df3 = pd.DataFrame()  #Temporary dataframe to capture info for each mask in the loop
    ...
    df3['Label_Value'] = label_values
    df3['file_name'] = mask
    
    mask_dataset = mask_dataset.append(df3)  #Update mask dataframe with all the info from each mask
...
X = dataset.drop(labels = ['file_name', 'Label_Value', 'x', 'y'], axis=1) 
